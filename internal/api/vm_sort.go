...
func parseVMSort(r *http.Request) (sortField, order string, err error) {
	...
	if !types.IsValidVMSort(sortField) {
		return "", "", types.NewAPIError(
			"invalid_sort",
			"sort must be one of: id, name, created_at, state, cpus, ram_mb, disk_gb, ip, image, default_user, gpu, os_type, firmware, os_variant, disk_bus, nic_model, machine, clock_offset, auto_start, locked",
		)
	}
