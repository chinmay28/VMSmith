...
// parseSnapshotSort reads `?sort=` and `?order=` from the request, validates
// them against the whitelisted set, and returns the canonical (lowercase)
// values or a typed *APIError for the handler to surface as 400. Mirrors
// `parseVMSort` / `parseImageSort`.
//
...
func parseSnapshotSort(r *http.Request) (sortField, order string, err error) {
