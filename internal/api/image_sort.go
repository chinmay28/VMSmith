...
// parseImageSort reads `?sort=` and `?order=` from the request, validates them
// against the whitelisted set, and returns the canonical (lowercase) values
// or a typed *APIError for the handler to surface as 400. Mirrors
// `parseVMSort`.
//
...
func parseImageSort(r *http.Request) (sortField, order string, err error) {
	...
	case types.ImageSortID, types.ImageSortName, types.ImageSortSize, types.ImageSortCreatedAt,
		types.ImageSortSourceVM, types.ImageSortDescription:
	default:
		return "", "", types.NewAPIError(
			"invalid_sort",
			"sort must be one of: id, name, size, created_at, source_vm, description",
		)
	}
