...
func TestValidateTarget_RejectsNonHTTPScheme(t *testing.T) {
	_, _, err := validateTarget("ftp://example.com/x", nil, nil)
...
func TestValidateTarget_RejectsLoopback(t *testing.T) {
	_, _, err := validateTarget("http://localhost/hook", nil, staticResolver("127.0.0.1"))
...
func TestValidateTarget_RejectsLinkLocalMetadata(t *testing.T) {
	_, _, err := validateTarget("http://metadata.example/x", nil, staticResolver("169.254.169.254"))
	if !errors.Is(err, ErrSSRFBlocked) {
		t.Fatalf("err = %v, want ErrSSRFBlocked", err)
	}
}
...
func TestValidateTarget_RejectsVMNATRange(t *testing.T) {
	_, _, err := validateTarget("http://nat.example/x", nil, staticResolver("192.168.100.10"))
	if !errors.Is(err, ErrSSRFBlocked) {
		t.Fatalf("err = %v, want ErrSSRFBlocked", err)
	}
}
...
func TestValidateTarget_RejectsIPv6Loopback(t *testing.T) {
	_, _, err := validateTarget("http://v6.example/x", nil, staticResolver("::1"))
	if !errors.Is(err, ErrSSRFBlocked) {
		t.Fatalf("err = %v, want ErrSSRFBlocked", err)
	}
}
...
func TestValidateTarget_AllowsPublicAddress(t *testing.T) {
	_, _, err := validateTarget("https://example.com/hook", nil, staticResolver("93.184.216.34"))
	if err != nil {
...
func TestValidateTarget_AllowedHostBypassesDeny(t *testing.T) {
	...
	_, _, err := validateTarget("http://localhost:9000/hook",
		[]string{"localhost"},
...
func TestValidateTarget_DNSFailureRejected(t *testing.T) {
	...
	_, _, err := validateTarget("http://nope.example/x", nil, failing)
	if err == nil {
