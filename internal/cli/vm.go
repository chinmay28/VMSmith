...
var vmListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if !types.IsValidVMSort(sortField) {
			return fmt.Errorf("invalid --sort %q: must be one of id, name, created_at, state, cpus, ram_mb, disk_gb, ip, image, default_user, gpu, os_type", sortField)
		}
...
Examples:
  ...
  --network eth1 --network eth2 --network eth3`)
	...
	vmListCmd.Flags().String("sort", types.VMSortID, "sort field: id, name, created_at, state, cpus, ram_mb, disk_gb, ip, image, default_user, gpu, os_type")
