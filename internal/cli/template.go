...
var templateListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if !types.IsValidTemplateSort(sortField) {
			return fmt.Errorf("invalid --sort %q: must be one of id, name, created_at, cpus, ram_mb, disk_gb, image, default_user, os_type", sortField)
		}
...
func init() {
	...
	templateListCmd.Flags().String("sort", types.TemplateSortID, "sort field: id, name, created_at, cpus, ram_mb, disk_gb, image, default_user, os_type")
