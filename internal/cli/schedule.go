...
var scheduleListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if sortField != "" && !types.IsValidScheduleSort(sortField) {
			return fmt.Errorf("invalid --sort: must be one of id, name, created_at, next_fire_at, last_fired_at, vm_id, action, timezone, enabled, catch_up_policy")
		}
...
func init() {
	...
	scheduleListCmd.Flags().String("sort", "", "sort field: id|name|created_at|next_fire_at|last_fired_at|vm_id|action|timezone|enabled|catch_up_policy (default id)")
