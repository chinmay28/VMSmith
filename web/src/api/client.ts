...
// --- VMs ---
export const vms = {
  list: ({ tag = '', status = '', search = '', image = '', defaultUser = '', osType = '', osVariant = '', firmware = '', diskBus = '', nicModel = '', machine = '', clockOffset = '', network = '', prefix = '', natStaticIp = '', natGateway = '', autoStart = '', locked = '', since = '', until = '', minCpus = '', maxCpus = '', minRamMb = '', maxRamMb = '', minDiskGb = '', maxDiskGb = '', sort = '', order = '', page, perPage }: { tag?: string; status?: string; search?: string; image?: string; defaultUser?: string; osType?: 'linux' | 'windows' | ''; osVariant?: 'windows-10' | 'windows-11' | 'windows-server-2019' | 'windows-server-2022' | 'windows-server-2025' | ''; firmware?: 'bios' | 'uefi' | 'ovmf' | ''; diskBus?: 'virtio' | 'sata' | ''; nicModel?: 'virtio' | 'e1000e' | ''; machine?: string; clockOffset?: 'utc' | 'localtime' | ''; network?: string; prefix?: string; natStaticIp?: string; natGateway?: string; autoStart?: 'true' | 'false' | ''; locked?: 'true' | 'false' | ''; since?: string; until?: string; minCpus?: string; maxCpus?: string; minRamMb?: string; maxRamMb?: string; minDiskGb?: string; maxDiskGb?: string; sort?: 'id' | 'name' | 'created_at' | 'state' | ''; order?: 'asc' | 'desc' | ''; page?: number; perPage?: number } = {}) =>
    unwrap(apiClient.GET('/vms', { params: { query: { tag, status, search: search || undefined, image: image || undefined, default_user: defaultUser || undefined, os_type: osType || undefined, os_variant: osVariant || undefined, firmware: firmware || undefined, disk_bus: diskBus || undefined, nic_model: nicModel || undefined, machine: machine || undefined, clock_offset: clockOffset || undefined, network: network || undefined, prefix: prefix || undefined, nat_static_ip: natStaticIp || undefined, nat_gateway: natGateway || undefined, auto_start: autoStart || undefined, locked: locked || undefined, since: since || undefined, until: until || undefined, min_cpus: minCpus || undefined, max_cpus: maxCpus || undefined, min_ram_mb: minRamMb || undefined, max_ram_mb: maxRamMb || undefined, min_disk_gb: minDiskGb || undefined, max_disk_gb: maxDiskGb || undefined, sort: sort || undefined, order: order || undefined, page, per_page: perPage } as any } }), { withMeta: true }),
