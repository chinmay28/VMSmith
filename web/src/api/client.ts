...
export const schedules = {
  ...
  runs: (id: string, { page, perPage, status = '', vmId = '', since = '', until = '', finishedSince = '', finishedUntil = '', minDurationMs, maxDurationMs, search = '', sort = '', order = '' }: { page?: number; perPage?: number; status?: 'running' | 'success' | 'error' | 'skipped' | ''; vmId?: string; since?: string; until?: string; finishedSince?: string; finishedUntil?: string; minDurationMs?: number; maxDurationMs?: number; search?: string; sort?: 'id' | 'started_at' | 'finished_at' | 'status' | 'duration' | ''; order?: 'asc' | 'desc' | '' } = {}) =>
