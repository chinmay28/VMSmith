...
export const schedules = {
  list: ({ page, perPage, vmId = '', tagSelector = '', action = '', catchUpPolicy = '', timezone = '', enabled = '', search = '', since = '', until = '', sort = '', order = '' }: { page?: number; perPage?: number; vmId?: string; tagSelector?: string; action?: 'snapshot' | 'start' | 'stop' | 'restart' | ''; catchUpPolicy?: 'skip' | 'run_once' | 'run_all' | ''; timezone?: string; enabled?: 'true' | 'false' | ''; search?: string; since?: string; until?: string; sort?: 'id' | 'name' | 'created_at' | 'next_fire_at' | ''; order?: 'asc' | 'desc' | '' } = {}) =>
