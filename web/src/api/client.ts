...
// --- VMs ---
export const vms = {
  list: ({ tag = '', status = '', search = '', image = '', defaultUser = '', osType = '', network = '', autoStart = '', locked = '', since = '', until = '', minCpus = '', maxCpus = '', minRamMb = '', maxRamMb = '', minDiskGb = '', maxDiskGb = '', sort = '', order = '', page, perPage }: { tag?: string; status?: string; search?: string; image?: string; defaultUser?: string; osType?: 'linux' | 'windows' | ''; network?: string; autoStart?: 'true' | 'false' | ''; locked?: 'true' | 'false' | ''; since?: string; until?: string; minCpus?: string; maxCpus?: string; minRamMb?: string; maxRamMb?: string; minDiskGb?: string; maxDiskGb?: string; sort?: 'id' | 'name' | 'created_at' | 'state' | ''; order?: 'asc' | 'desc' | ''; page?: number; perPage?: number } = {}) =>
    unwrap(apiClient.GET('/vms', { params: { query: { tag, status, search: search || undefined, image: image || undefined, default_user: defaultUser || undefined, os_type: osType || undefined, network: network || undefined, auto_start: autoStart || undefined, locked: locked || undefined, since: since || undefined, until: until || undefined, min_cpus: minCpus || undefined, max_cpus: maxCpus || undefined, min_ram_mb: minRamMb || undefined, max_ram_mb: maxRamMb || undefined, min_disk_gb: minDiskGb || undefined, max_disk_gb: maxDiskGb || undefined, sort: sort || undefined, order: order || undefined, page, per_page: perPage } as any } }), { withMeta: true }),
...
// --- Images ---
export const images = {
  list: ({ page, perPage, tag = '', sourceVM = '', search = '', since = '', until = '', minSize = '', maxSize = '', sort, order }: { page?: number; perPage?: number; tag?: string; sourceVM?: string; search?: string; since?: string; until?: string; minSize?: string; maxSize?: string; sort?: string; order?: string } = {}) =>
    unwrap(apiClient.GET('/images', { params: { query: { page, per_page: perPage, tag: tag || undefined, source_vm: sourceVM || undefined, search: search || undefined, since: since || undefined, until: until || undefined, min_size: minSize || undefined, max_size: maxSize || undefined, sort, order } as any } }), { withMeta: true }),
...
export const templates = {
  list: ({ page, perPage, tag, search, image, defaultUser, osType, network, since, until, minCpus, maxCpus, minRamMb, maxRamMb, minDiskGb, maxDiskGb, sort, order }: { page?: number; perPage?: number; tag?: string; search?: string; image?: string; defaultUser?: string; osType?: 'linux' | 'windows' | ''; network?: string; since?: string; until?: string; minCpus?: string; maxCpus?: string; minRamMb?: string; maxRamMb?: string; minDiskGb?: string; maxDiskGb?: string; sort?: string; order?: string } = {}) =>
    unwrap(apiClient.GET('/templates', { params: { query: { page, per_page: perPage, tag, search, image, default_user: defaultUser || undefined, os_type: osType || undefined, network: network || undefined, since: since || undefined, until: until || undefined, min_cpus: minCpus || undefined, max_cpus: maxCpus || undefined, min_ram_mb: minRamMb || undefined, max_ram_mb: maxRamMb || undefined, min_disk_gb: minDiskGb || undefined, max_disk_gb: maxDiskGb || undefined, sort, order } as any } } as any), { withMeta: true }),
...
export const schedules = {
  list: ({ page, perPage, vmId = '', tagSelector = '', action = '', catchUpPolicy = '', timezone = '', enabled = '', search = '', since = '', until = '', nextFireSince = '', nextFireUntil = '', sort = '', order = '' }: { page?: number; perPage?: number; vmId?: string; tagSelector?: string; action?: 'snapshot' | 'start' | 'stop' | 'restart' | ''; catchUpPolicy?: 'skip' | 'run_once' | 'run_all' | ''; timezone?: string; enabled?: 'true' | 'false' | ''; search?: string; since?: string; until?: string; nextFireSince?: string; nextFireUntil?: string; sort?: 'id' | 'name' | 'created_at' | 'next_fire_at' | ''; order?: 'asc' | 'desc' | '' } = {}) =>
    unwrap(apiClient.GET('/schedules', { params: { query: { vm_id: vmId || undefined, tag_selector: tagSelector || undefined, action: action || undefined, catch_up_policy: catchUpPolicy || undefined, timezone: timezone || undefined, enabled: enabled || undefined, search: search || undefined, since: since || undefined, until: until || undefined, next_fire_since: nextFireSince || undefined, next_fire_until: nextFireUntil || undefined, sort: sort || undefined, order: order || undefined, page, per_page: perPage } as any } }), { withMeta: true }),
  ...
  runs: (id: string, { page, perPage, status = '', vmId = '', since = '', until = '', finishedSince = '', finishedUntil = '', search = '', sort = '', order = '' }: { page?: number; perPage?: number; status?: 'running' | 'success' | 'error' | 'skipped' | ''; vmId?: string; since?: string; until?: string; finishedSince?: string; finishedUntil?: string; search?: string; sort?: 'id' | 'started_at' | 'finished_at' | 'status' | 'duration' | ''; order?: 'asc' | 'desc' | '' } = {}) =>
    unwrap(apiClient.GET('/schedules/{scheduleID}/runs', { params: { path: { scheduleID: id }, query: { status: status || undefined, vm_id: vmId || undefined, since: since || undefined, until: until || undefined, finished_since: finishedSince || undefined, finished_until: finishedUntil || undefined, search: search || undefined, sort: sort || undefined, order: order || undefined, page, per_page: perPage } as any } }), { withMeta: true }),
  runNow: (id: string) =>
