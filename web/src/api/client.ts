...
export const schedules = {
  list: ({ page, perPage, vmId = '', action = '', enabled = '', search = '', since = '', until = '', sort = '', order = '' }: { page?: number; perPage?: number; vmId?: string; action?: 'snapshot' | 'start' | 'stop' | 'restart' | ''; enabled?: 'true' | 'false' | ''; search?: string; since?: string; until?: string; sort?: 'id' | 'name' | 'created_at' | 'next_fire_at' | ''; order?: 'asc' | 'desc' | '' } = {}) =>
    unwrap(apiClient.GET('/schedules', { params: { query: { vm_id: vmId || undefined, action: action || undefined, enabled: enabled || undefined, search: search || undefined, since: since || undefined, until: until || undefined, sort: sort || undefined, order: order || undefined, page, per_page: perPage } as any } }), { withMeta: true }),
