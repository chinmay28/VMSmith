...
// --- VMs ---
export const vms = {
  list: ({ tag = '', status = '', search = '', image = '', defaultUser = '', network = '', autoStart = '', locked = '', since = '', until = '', minCpus = '', maxCpus = '', minRamMb = '', maxRamMb = '', sort = '', order = '', page, perPage }: { tag?: string; status?: string; search?: string; image?: string; defaultUser?: string; network?: string; autoStart?: 'true' | 'false' | ''; locked?: 'true' | 'false' | ''; since?: string; until?: string; minCpus?: string; maxCpus?: string; minRamMb?: string; maxRamMb?: string; sort?: 'id' | 'name' | 'created_at' | 'state' | ''; order?: 'asc' | 'desc' | ''; page?: number; perPage?: number } = {}) =>
    unwrap(apiClient.GET('/vms', { params: { query: { tag, status, search: search || undefined, image: image || undefined, default_user: defaultUser || undefined, network: network || undefined, auto_start: autoStart || undefined, locked: locked || undefined, since: since || undefined, until: until || undefined, min_cpus: minCpus || undefined, max_cpus: maxCpus || undefined, min_ram_mb: minRamMb || undefined, max_ram_mb: maxRamMb || undefined, sort: sort || undefined, order: order || undefined, page, per_page: perPage } as any } }), { withMeta: true }),
