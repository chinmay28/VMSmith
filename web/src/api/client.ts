...
// --- VMs ---
export const vms = {
  list: ({ tag = '', status = '', search = '', autoStart = '', locked = '', sort = '', order = '', page, perPage }: { tag?: string; status?: string; search?: string; autoStart?: 'true' | 'false' | ''; locked?: 'true' | 'false' | ''; sort?: 'id' | 'name' | 'created_at' | 'state' | ''; order?: 'asc' | 'desc' | ''; page?: number; perPage?: number } = {}) =>
    unwrap(apiClient.GET('/vms', { params: { query: { tag, status, search: search || undefined, auto_start: autoStart || undefined, locked: locked || undefined, sort: sort || undefined, order: order || undefined, page, per_page: perPage } as any } }), { withMeta: true }),
