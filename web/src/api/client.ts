...
// --- VMs ---
export const vms = {
  list: ({ tag = '', status = '', search = '', image = '', defaultUser = '', osType = '', osVariant = '', firmware = '', diskBus = '', nicModel = '', machine = '', clockOffset = '', network = '', prefix = '', natStaticIp = '', autoStart = '', locked = '', since = '', until = '', minCpus = '', maxCpus = '', minRamMb = '', maxRamMb = '', minDiskGb = '', maxDiskGb = '', sort = '', order = '', page, perPage }: { tag?: string; status?: string; search?: string; image?: string; defaultUser?: string; osType?: 'linux' | 'windows' | ''; osVariant?: 'windows-10' | 'windows-11' | 'windows-server-2019' | 'windows-server-2022' | 'windows-server-2025' | ''; firmware?: 'bios' | 'uefi' | 'ovmf' | ''; diskBus?: 'virtio' | 'sata' | ''; nicModel?: 'virtio' | 'e1000e' | ''; machine?: string; clockOffset?: 'utc' | 'localtime' | ''; network?: string; prefix?: string; natStaticIp?: string; autoStart?: 'true' | 'false' | ''; locked?: 'true' | 'false' | ''; since?: string; until?: string; minCpus?: string; maxCpus?: string; minRamMb?: string; maxRamMb?: string; minDiskGb?: string; maxDiskGb?: string; sort?: 'id' | 'name' | 'created_at' | 'state' | ''; order?: 'asc' | 'desc' | ''; page?: number; perPage?: number } = {}) =>
