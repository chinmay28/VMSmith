        ...
        get: {
            parameters: {
                query?: {
                     ...
                     *     (5.4.109) and the schedule `enabled` axis (5.4.113).
                     *     `description` (5.4.122) is a case-insensitive sort axis over
                     *     the webhook's `description` field — operators paste
                     *     descriptions verbatim into list-search filters, so the sort
                     *     axis matches the same case-insensitive semantics as the
                     *     existing `?search=` filter haystack on the same column;
                     *     webhooks with an empty `description` (the common case — most
                     *     webhooks get no description) sink to the tail of `asc` and
                     *     the head of `desc`, mirroring the nullable-string nil-trailing
                     *     convention on every other nullable axis (`url`,
                     *     `last_delivery_at`) and the VM list `description` axis
                     *     (5.4.120) / template list `description` axis (5.4.119) /
                     *     image list `description` axis (5.4.118) / snapshot list
                     *     `description` axis (5.4.121) one resource over. Closes the
                     *     operator query *"which webhooks have a description so I can
                     *     audit the documented receivers"* — they cluster at the head
                     *     of `asc` instead of being buried among the unset majority.
                     *     All comparators tiebreak on `id` so repeated requests return
                     *     a deterministic order.
                     */
                    sort?: "id" | "url" | "created_at" | "last_delivery_at" | "delivery_status" | "active" | "description";
...
export interface components {
    ...
    parameters: {
         ...
         *     requests return the same set across two independent fetches.
         */
        TemplateSort: "id" | "name" | "created_at" | "cpus" | "ram_mb" | "disk_gb" | "image" | "default_user" | "os_type" | "os_variant" | "description";
        /**
         * @description Sort direction. Defaults to `asc`. Unknown values return 400
         *     `invalid_order`.
         */
