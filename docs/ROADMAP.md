...
| 4.2.4 | Refactor `internal/vm/events.go` to call `bus.Publish` for each lifecycle callback; move state mutation into a separate `persistVMState` consumer subscribed to `libvirt` events. Removes the direct `store.PutVM` call from the libvirt goroutine. | S | ✅ Done — `handleLifecycleEvent` now translates each libvirt callback into a typed `Event` with `Source=libvirt`, dotted type (e.g. `vm.started`, `vm.crashed`), and severity, and publishes to the bus. State persistence moved to `vm.VMStatePersister`, a `bus.Subscribe`-driven consumer started by the daemon after `bus.Start()`; the libvirt goroutine no longer writes to bbolt directly |
| 4.2.5 | Promote the existing `events` bucket to the indexed scheme: add `events_meta` (next_id), key events by big-endian `uint64` ID, expose `Store.ListEvents(filter, limit)`, `Store.EventByID(id)`, `Store.PruneEvents(maxRecords, maxAge)` | M | ✅ Done — indexed storage, filtered listing, replay-by-seq, record-count pruning, and age-based pruning (`Store.PruneEventsByAge`) all live in `internal/store/bolt.go` |
