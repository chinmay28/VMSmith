...
| 3.1.4 | Add login screen to frontend when auth is enabled | M | ✅ Done — React app now prompts for an API key on 401, stores it in `localStorage`, and retries API requests with `Authorization: Bearer <key>` |
