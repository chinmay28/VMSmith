...
| 4.2.14 | CLI: `vmsmith events list [--vm <id>] [--type <t>] [--source <s>] [--severity <sev>] [--since <duration|id>] [--limit <n>]` (one-shot REST query) and `vmsmith events follow [--vm <id>] [--type <t>]` (SSE, prints events as they arrive, exits on Ctrl-C) | M | Partially done — `vmsmith events list` is shipped (`internal/cli/events.go`) with `--vm/--type/--source/--severity/--since/--limit` filters. `events follow` deferred until the SSE endpoint (4.2.10) lands |
