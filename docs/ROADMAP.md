...
| 4.2.6 | Emit `app`-source events from API handlers: VM create / clone / update / delete, snapshot create / restore / delete, image upload / delete, port forward add / remove. Provide an `events.PublishAppEvent(ctx, type, vmID, attrs)` helper that pulls the actor from the auth middleware context | M | ✅ Done — VM create / update / clone / delete / start / stop, snapshot create / restore / delete, image create / upload / delete, and port-forward add / remove handlers now publish via `Server.publishAppEvent` (no-op when bus unwired). Actor capture via auth-context still pending (revisit when 3.1 lands) |
| 4.2.7 | Emit `system`-source events for daemon start / shutdown, quota exceeded, DHCP exhaustion, port-forward restore failure | S | ✅ Done — `daemon.started` (with listen + tls attrs) and `daemon.shutdown` are now published from `internal/daemon/daemon.go`. Retention sweeps emit `events.retention_pruned`. quota / DHCP / port-forward-restore failure publishes still pending |
| 4.2.8 | Retention loop (every 60s) honoring `daemon.events.max_records` (default 50_000) and `daemon.events.max_age` (default 720h); cap deletes per sweep at 5000; emit `system` event when retention drops events | S | ✅ Done — `internal/events/retention.go` polls `Store.PruneEvents` at the configured interval, emits `events.retention_pruned` (system, info) on each non-zero sweep, and is wired into `daemon.Run` honouring `events.max_records` / `events.retention_interval` config. `max_age` knob is parked behind a follow-up (current `PruneEvents` is record-count-only) |
...
| 4.2.11 | Frontend: `web/src/hooks/useEventStream.js` — opens `EventSource`, handles reconnect with `Last-Event-ID`, falls back to polling on 410 / network error for 30s, exposes connection state. Replace polling on Dashboard and VMList with the hook; add a small "live" indicator | M | ✅ Done — `web/src/hooks/useEventStream.js` opens `/api/v1/events/stream` via `EventSource`, resumes via `?since=<lastID>` (browsers can't set custom headers), falls back to short-poll mode after 3 consecutive failures or HTTP 410, and exposes `status` (`connecting`/`live`/`reconnecting`/`fallback`/`closed`). Dashboard + VMList drop their 5s poll to 30s, refetch on `vm.*` / `image.*` events, and surface a `LiveIndicator` pill in `Shared.jsx`. Mock SSE endpoint added to `tests/web/mock-server.js` plus runner cases in `tests/web/run-gui-tests.js` and Playwright cases in `tests/web/gui.spec.js` |
