...
| 2.3.6 | Bulk add / remove / set tags across multiple VMs in one request (`POST /api/v1/vms/bulk_tag` with `{action: "add"\|"remove"\|"set", ids: [...], tags: [...]}`), CLI (`vmsmith vm tag <add\|remove\|set> <id[,id...]> --tag t1` plus `--all [--filter-tag <t>]` for fleet-wide application), and GUI (Tags ▾ dropdown on the bulk action bar with per-action modal). Tag mutation semantics (case-insensitive de-dup for add, case-insensitive filter for remove, replacement for set, alphabetic-sort canonical ordering) live in `pkg/types.MergeTags` so the API and CLI share one implementation. | S | ✅ Done — `internal/api/handlers_vm_bulk_tag.go` ships the endpoint with typed `invalid_bulk_action` / `invalid_bulk_request` 400 codes; per-VM results follow the same `{id, success, code?, message?}` shape as `/vms/bulk` and `/vms/{id}/snapshots/bulk_delete`, plus a `tags` field carrying the post-update tag set. The handler short-circuits when the proposed change is observably a no-op (case-insensitive set equality via `pkg/types.TagsEqualSet`) so unchanged VMs skip a redundant `Update` call. `internal/cli/vm.go::vmTagCmd` mirrors the API surface for in-process libvirt use. `web/src/pages/VMList.jsx` adds `BulkTagMenu` + `BulkTagModal` driven by `vms.bulkTag`. Each successful target emits a `vm.tags_updated` app event. Coverage: 7 API integration cases (`TestBulkVMTag_*` covering add/remove/set happy paths, case-insensitive de-dup no-op, partial failure, invalid action / empty ids / empty add-or-remove tags, manager error), 12 `pkg/types.MergeTags` / `TagsEqualSet` unit cases, 7 CLI cases (`TestCLI_VMTag_*`), and 2 Playwright scenarios in `tests/web/run-gui-tests.js` (`bulk tag add applies tags…`, `bulk tag set with empty input clears all tags`). OpenAPI / TS schema regenerated. |
