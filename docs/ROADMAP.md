...
| 1.2.5 | Standardize error responses: introduce error codes (`invalid_name`, `resource_not_found`, `disk_shrink_not_allowed`, etc.) | M | ✅ Done — `pkg/types/errors.go` carries typed API error codes and the handlers now return normalized `APIError` payloads for client-visible failures across VM, network, image, and snapshot routes |
