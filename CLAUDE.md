...
GET    /templates                      List templates. Filters: `?tag=<tag>` (case-insensitive, applied before sort + pagination). Sorting: `?sort=<id|name|created_at>` (default `id`) and `?order=<asc|desc>` (default `asc`); unknown values return 400 `invalid_sort` / `invalid_order`. All comparators tiebreak on `id` so paginated responses are deterministic. Pagination: `?page=<n>&per_page=<n>`; returns `X-Total-Count`. CLI mirrors via `vmsmith template list --tag --sort --order --limit --offset`
