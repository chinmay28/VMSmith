...
GET    /events                         List events (newest-first by default). Filters: `?vm_id=`, `?type=`, `?source=`, `?severity=`, `?resource_id=<id>` (exact match, case-sensitive — mirrors the `vm_id` contract; operators reference opaque server-issued IDs verbatim), `?since=<rfc3339>`, `?until=<seq>`, and `?search=<text>` — a case-insensitive substring match across `message`, `type`, `source`, `severity`, `actor`, `vm_id`, `resource_id`, and every value in `attributes`. The numeric event ID is intentionally excluded from the haystack. Sorting: `?sort=<id|occurred_at|type|source|severity>` (default `id`) and `?order=<asc|desc>` (default `desc`). `type`, `source`, and `severity` match case-insensitively; all comparators tiebreak on `id` so paginated responses are deterministic. Unknown values return 400 `invalid_sort` / `invalid_order`. Filters compose additively; CLI mirrors via `vmsmith events list --search --sort --order --resource-id --actor --attrs` (and `events follow --search --resource-id --actor --attrs` for the SSE stream). Opt-in `--actor` adds an ACTOR column between TYPE and VM; `--attrs` adds a trailing ATTRIBUTES column that folds in `resource_id` plus sorted `key=value` pairs from the structured `attributes` map. Activity page in the GUI surfaces the same fields via a per-row chevron toggle that expands a details sub-row.
