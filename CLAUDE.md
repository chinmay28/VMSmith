...
│   │   ├── network.go           # vmsmith port add|edit|remove|list (`port remove` accepts a positional id for single-delete, or `--vm <id> [--protocol tcp|udp]` to bulk-delete every rule on a VM; `port edit <id> --description "..."` mutates the description — pass `""` to clear)
...
GET    /vms/{id}/snapshots             List snapshots (each entry carries `name`, optional `description`, and a libvirt-parsed `created_at`). Sorting: `?sort=<id|name|created_at>` (default `id`) and `?order=<asc|desc>` (default `asc`); unknown values return 400 `invalid_sort` / `invalid_order`. Within a VM the snapshot ID is `<vmID>/<name>` so id-asc == name-asc; all comparators tiebreak on `name` so paginated responses are deterministic. Filtering: `?search=<text>` — a case-insensitive substring match across `name` and `description` (the haystack excludes the snapshot ID and `vm_id` to avoid noisy false positives on short numeric queries); whitespace-trimmed; applied before sort + pagination so `X-Total-Count` reflects the post-search count. Filters compose additively with `sort` / `order` / pagination. CLI mirrors via `vmsmith snapshot list <vm-id> --sort --order --search`.
...
GET    /vms/{id}/ports                 List port forwards. Sorting: `?sort=<id|host_port|guest_port|protocol|description>` (default `id`) and `?order=<asc|desc>` (default `asc`). The `description` sort is case-insensitive; unknown values return 400 `invalid_sort` / `invalid_order`. All comparators tiebreak on `id` so repeated requests return a deterministic order. Free-text filter: `?search=<text>` — case-insensitive substring match across `description`, `protocol`, `host_port`, `guest_port`, and `guest_ip`; whitespace-trimmed; rule ID and `vm_id` are intentionally excluded from the haystack. Filters compose additively with `sort` / `order`. CLI mirrors via `vmsmith port list <vm-id> --sort --order --search`
...
PATCH  /vms/{id}/ports/{portId}        Update editable port-forward metadata (currently only `description`; ≤256 chars). Body: `{"description": "..."}` — empty string clears, missing/null leaves unchanged. The iptables 5-tuple (host_port/guest_port/guest_ip/protocol) is intentionally immutable. The URL VM is the authoritative scope: a `portId` belonging to a different VM returns 404 `resource_not_found`. Emits `port_forward.updated`. CLI: `vmsmith port edit <port-id> --description "..."`.
...
GET    /logs                           Query log entries. Filters: `level`, `source`, `since`, `limit`/`per_page`, and `?search=<q>` — a case-insensitive substring match across the entry's `message`, `source`, `level`, and every *value* in the structured fields map. Whitespace-trimmed; field **keys** are intentionally excluded (small repeating vocabulary like `vm_id` / `method` / `error` would generate noisy matches), timestamps are scoped via `since`. Sorting: `?sort=<timestamp|level|source>` (default `timestamp`) and `?order=<asc|desc>` (default `asc` — preserves the legacy oldest-first contract). `level` orders by severity rank (debug < info < warn < error), not alphabetically; `source` matches case-insensitively. Unknown values return 400 `invalid_sort` / `invalid_order`. All comparators tiebreak on timestamp+source so paginated responses are deterministic. Filters compose additively, so `X-Total-Count` reflects the post-search population. CLI mirrors via `vmsmith logs list --level --source --since --search --sort --order --limit --page --fields` (talks to the daemon's ring buffer over HTTP; reuses the persistent `--api-url` / `--api-key` flags).
GET    /events                         List events (newest-first by default). Filters: `?vm_id=`, `?type=`, `?source=`, `?severity=`, `?since=<rfc3339>`, `?until=<seq>`, and `?search=<text>` — a case-insensitive substring match across `message`, `type`, `source`, `severity`, `actor`, `vm_id`, `resource_id`, and every value in `attributes`. The numeric event ID is intentionally excluded from the haystack. Sorting: `?sort=<id|occurred_at|type|source|severity>` (default `id`) and `?order=<asc|desc>` (default `desc`). `type`, `source`, and `severity` match case-insensitively; all comparators tiebreak on `id` so paginated responses are deterministic. Unknown values return 400 `invalid_sort` / `invalid_order`. Filters compose additively; CLI mirrors via `vmsmith events list --search --sort --order` (and `events follow --search` for the SSE stream).
GET    /webhooks                       List registered webhooks (secrets redacted). Filter: `?search=<text>` — case-insensitive substring match across `url`, `description`, and `event_types`; whitespace-trimmed; `id`, `secret`, and `last_error` are intentionally excluded from the haystack. Sorting: `?sort=<id|url|created_at|last_delivery_at>` (default `id`) and `?order=<asc|desc>` (default `asc`); unknown values return 400 `invalid_sort` / `invalid_order`. `url` matches case-insensitively; `last_delivery_at` puts never-delivered webhooks at the tail of `asc` / head of `desc`. All comparators tiebreak on `id` so paginated responses are deterministic. CLI mirrors via `vmsmith webhook list --search --sort --order`
POST   /webhooks                       Register webhook (URL + HMAC secret + optional `event_types` filters + optional `description` ≤1024 chars)
PATCH  /webhooks/{id}                  Update editable fields on an existing webhook (`url`, `secret`, `event_types`, `active`, `description`). All five fields use pointer semantics — omit a key to leave it unchanged; `event_types: []` clears the filter so the webhook matches every event again; `description: ""` clears the description (trimmed; capped at 1024 chars); rotating `secret` cannot be cleared (empty string returns 400 `missing_secret`). The in-memory delivery worker is bounced on success so the next event is delivered with the new config. Returns 400 `noop_update` when no editable field is present. CLI: `vmsmith webhook edit <id> [--url --secret --event-types --clear-event-types --active --description]`.
