...
```
GET    /vms                            List all VMs. Filters: `?tag=<tag>`, `?status=<state>`, `?search=<text>` (case-insensitive substring across name/description/tags; whitespace-trimmed; ID intentionally excluded — combine with `tag` / `status` for narrower scoping). Sorting: `?sort=<id|name|created_at|state>` (default `id`) and `?order=<asc|desc>` (default `asc`); unknown values return 400 `invalid_sort` / `invalid_order`. All comparators tiebreak on `id` so paginated responses are deterministic. CLI mirrors via `vmsmith vm list --tag --status --search --sort --order --limit --offset`
