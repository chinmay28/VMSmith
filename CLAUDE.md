...
│   │   ├── network.go           # vmsmith port add|edit|remove|list (`port remove` accepts a positional id for single-delete, or `--vm <id> [--protocol tcp|udp]` to bulk-delete every rule on a VM; `port add --tag <t>` and `port edit <id> --description "..." --tag <t> --clear-tags` mutate the description and tag set — pass `""` to clear description, `--clear-tags` to drop every tag; `port list --tag <t>` filters by tag and prints a TAGS column; `port list --min-host-port <n> --max-host-port <n>` filters by an inclusive host_port range)
...
GET    /schedules/{id}/runs            List the schedule's run history (newest first; `?page=&per_page=`; `X-Total-Count`). Each run: `{id, schedule_id, vm_id, started_at, finished_at?, status (running|success|error|skipped), error?, skip_reason?}`. 404 if the schedule is unknown. Filters (applied before pagination so `X-Total-Count` reflects the post-filter population): `?status=<running|success|error|skipped>` (case-insensitive exact-match; empty disables; unknown → 400 `invalid_status`), `?since=<rfc3339>` / `?until=<rfc3339>` (inclusive bounds on `started_at`; whitespace-trimmed; invalid → 400 `invalid_since` / `invalid_until`; a run with a zero `started_at` is filtered OUT when any bound is set — mirrors the schedule/snapshot/image time-range family). CLI: `vmsmith schedule runs <id> --status --since --until --limit --page`; GUI: a per-row "Filter runs by status" dropdown on the Schedules page recent-runs expander.
