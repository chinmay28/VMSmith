...
GET    /events                         List events (newest-first by default). Filters: `?vm_id=`, `?type=`, `?source=`, `?severity=`, `?actor=<name>` — case-sensitive exact-match against the event's `actor` field (e.g. `system`, `app`, or an API-key alias); whitespace-trimmed; empty disables the filter; mirrors `?vm_id=`'s contract (case-insensitive matching is the job of `?search=`, not the exact-match filter). `?since=<rfc3339>`, `?until=<seq>`, and `?search=<text>` — a case-insensitive substring match across `message`, `type`, `source`, `severity`, `actor`, `vm_id`, `resource_id`, and every value in `attributes`. The numeric event ID is intentionally excluded from the haystack. Sorting: `?sort=<id|occurred_at|type|source|severity>` (default `id`) and `?order=<asc|desc>` (default `desc`). `type`, `source`, and `severity` match case-insensitively; all comparators tiebreak on `id` so paginated responses are deterministic. Unknown values return 400 `invalid_sort` / `invalid_order`. Filters compose additively; CLI mirrors via `vmsmith events list --actor --search --sort --order --show-actor --show-attrs` (and `events follow --actor --search --show-actor --show-attrs` for the SSE stream). Opt-in `--show-actor` adds an ACTOR column between TYPE and VM; `--show-attrs` adds a trailing ATTRIBUTES column that folds in `resource_id` plus sorted `key=value` pairs from the structured `attributes` map. Activity page in the GUI surfaces the same fields via a per-row chevron toggle that expands a details sub-row, plus a debounced "Filter by actor" input next to the VM-ID filter that round-trips through `?actor=` in the URL.
