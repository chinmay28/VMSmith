...
<!-- progress:start -->
**Overall:** 319 / 319 tasks complete (100.0%)

`▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓` 100.0%

| Phase | Title | Done | Total | % |
| ----- | ----- | ---- | ----- | - |
| 1 | Foundation & Quality (Week 1-2) | 18 | 18 | 100.0% |
| 2 | Core Feature Additions (Week 3-5) | 44 | 44 | 100.0% |
| 3 | Operational Excellence (Week 5-8) | 20 | 20 | 100.0% |
| 4 | Monitoring & Observability (Week 7-10) | 39 | 39 | 100.0% |
| 5 | Advanced Features (Week 10+) | 185 | 185 | 100.0% |
| 6 | Developer & Community (Ongoing) | 13 | 13 | 100.0% |
...
Source of truth: [`docs/ROADMAP.md`](docs/ROADMAP.md). The block above is regenerated automatically from the roadmap by `scripts/roadmap-progress.sh` (run on every push to `main` via `.github/workflows/roadmap-progress.yml`).

---

## Demos

Create a VM (with its IP assigned before first boot), snapshot it, and SSH in:

![vmsmith VM lifecycle demo](docs/demos/vm-lifecycle.svg)

Fleet operations — tag-scoped listing, recurring snapshot schedules, bulk restart, host capacity:

![vmsmith fleet operations demo](docs/demos/fleet-ops.svg)

*The demos are animated SVG "recordings" generated by
`scripts/generate-demos.py` — regenerate them after changing the scripted
sessions.*
